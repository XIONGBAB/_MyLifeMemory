        ...
        switch (listNode[i].className) {
          ...
          case "second_lists": {
            ...
            if (activeLink && activeLink !== this) {
              ...
              let firstHeight = parseFloat(
                third_container.parentNode.parentNode.children.length * this.querySelector(".third_container").children[0].offsetHeight
              );
              let secondHeight = parseFloat(
                this.querySelector(".third_container").children.length * this.querySelector(".third_container").children[0].offsetHeight
              );
            ...
            if (third_container.style.height === "0px" || third_container.style.height === "") {
              ...
              let linkId = "set_" + third_container.previousElementSibling.querySelector("span").innerText;
              third_container.id = linkId; // 绑定id
              if (isClicked) {
                this.parentNode.style.height = parseFloat(third_container.style.height) + parseFloat(listNode[i].parentNode.style.height) + "rem";
              } else {
                isClicked = true;
              }
    //           let cycleList = document.getElementById(linkId);
    // let programMenu = programming.map(e => {
    //         // return `?`;
    //     }).join("");
    // cycleList.innerHTML = programMenu;
            } else {
              activeLink = null;
              isClicked = true;
